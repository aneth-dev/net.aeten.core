...
public interface PlayerListener<T extends Record> {
	 ...
	 **/
	void position(Player<T, ?> player, Date date, double position);

	public abstract class Adapter<T extends Record> implements PlayerListener<T> {
		...
		@Override
		public void position(Player<T, ?> player, Date date, double position) {}
	}
}
