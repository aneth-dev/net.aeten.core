...
public class Document<T> {
	...
	public static class MappingEntry implements Map.Entry<Element, Element> {
		Element key;
		Element value;
	...
	public static class Element {
		...
		Element(Element parent, ElementType type, Object value, String valueType) {
			if (type == null) { throw new Error("Unable to create Element without type"); }
			this.parent = parent;
			...
			switch (type) {
			case SEQUENCE:
				if (value != null) { throw new IllegalArgumentException("Value must be null for " + type + ". " + value + " is given."); }
				...
				if (parent == null) {
					...
					case MAPPING_ENTRY: {
						...
						if (tag.getKey() == null) {
							...
							if (tag.getValue() != null) { throw new Error(String.format("Unable to insert element %s in tag. Key (%s) and value (%s) already defined", element.value, tag.getKey(), tag.getValue())); }
